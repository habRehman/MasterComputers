...
// Axios instance with short timeout so ML calls never hang the backend
const mlAxios = axios.create({ baseURL: ML_URL, timeout: 8000 })
...
router.get('/recommend', async (req, res) => {
  try {
    ...
    res.json({
      ...
      algorithm: 'category + price proximity',
    })
  } catch (err) {
...
router.post('/recommend/history', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ recommendations: recs, type: 'personalized' })
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
})
...
router.post('/chatbot', async (req, res) => {
  try {
    const mlRes = await mlAxios.post('/chatbot', { message: req.body.message })
    res.json(mlRes.data)
  } catch {
    res.json({
      type: 'error',
...
// GET /api/ml/predict-price/options
router.get('/predict-price/options', async (_req, res) => {
  try {
    const mlRes = await mlAxios.get('/predict-price/options')
    res.json(mlRes.data)
  } catch {
...
router.post('/predict-price', async (req, res) => {
  try {
    const mlRes = await mlAxios.post('/predict-price', req.body)
    res.json(mlRes.data)
  } catch (err) {
    res.status(503).json({ error: 'ML service unavailable. Deploy ml_service separately.' })
  }
})
...
router.get('/clusters', async (_req, res) => {
  try {
    const mlRes = await mlAxios.get('/clusters')
    res.json(mlRes.data)
  } catch {
    res.status(503).json({ error: 'ML service unavailable' })
  }
